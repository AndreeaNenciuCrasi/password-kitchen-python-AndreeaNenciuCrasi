...
def route_hash_password():
    password_input=request.args.get('password1')
    hashed_value=hash_password(password_input)
    # verify=verify_password(password_input, hashed_value)

    return render_template('password.html', hashed_value=hashed_value)


@app.route('/verify_hash_password')
def route_verify_hash_password():
    password_input_to_verify = request.args.get('password2')
